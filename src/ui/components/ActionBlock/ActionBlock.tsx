...
export default function ActionBlock(props: ActionBlockProps) {
  ...
  const [inputVariables, setInputVariables] = useState<SpotifyActionVariables>(new SpotifyActionVariables());

  const spotifyActionDropdownList: DropDownOption[] = (
    actionList.map((spotifyAction): DropDownOption => (
      {
        label: spotifyAction.label,
        value: spotifyAction.key.toString()
      }
  ...
  const validInputVariables: SpotifyActionVariable[] = (
    actions.filter((spotifyShortcutAction: SpotifyShortcutAction) => spotifyShortcutAction.index < actionIndex)
    // .filter((spotifyShortcutAction: SpotifyShortcutAction) => spotifyShortcutAction.outputs != undefined)
      .flatMap((spotifyShortcutAction: SpotifyShortcutAction) => spotifyShortcutAction.outputs as SpotifyActionVariables)
      .map((shortcutVariable: SpotifyActionVariable | undefined) => shortcutVariable as SpotifyActionVariable)
  )

  const validInputVariablesForType = (variableType: SpotifyActionArgument): SpotifyActionVariable[] => {
    return validInputVariables.filter((inputVariable: SpotifyActionVariable) => inputVariable?.type == variableType)
  }


  const inputVariablesAsDropdownList = (inputVariables: SpotifyActionVariable[]): DropDownOption[] => (
  ...
  const handleActionDropdownSelect = (option: string) => {
    ...
    setAction(new SpotifyShortcutAction(selectedAction.key, selectedAction.label, selectedAction.action, actionIndex, selectedAction.inputTypes, selectedAction.outputTypes));
  }

  const handleInputDropdownSelect = (option: string) => {
    const selectedInputVariable: SpotifyActionVariable = validInputVariables.filter((inputVariable: SpotifyActionVariable) => inputVariable.label == option).at(0) as SpotifyActionVariable;
  ...
  const getUndefinedInputTypes = (): Array<SpotifyActionArgument> => {
    const definedInputTypes: Array<SpotifyActionArgument> = action.inputs?.map((input: SpotifyActionVariable) => input.type) ?? [];
    const undefinedInputTypes = action.inputTypes?.filter((inputType: SpotifyActionArgument) =>
  ...
  const getSelectedInputOption = (inputType: SpotifyActionArgument): string => {
    const selectedInputVariable: SpotifyActionVariable = inputVariables.find((inputVariable: SpotifyActionVariable) => inputVariable.type == inputType) as SpotifyActionVariable;
  ...
  return (
    <div className={styles.container}>
      <div className={styles.actionDropdown}>
        <DropDownList extraProps={{ label: "Spotify Action", onChangeCallback: handleActionDropdownSelect, options: spotifyActionDropdownList, selectedOption: getSelectedActionOption()}} />
      </div> 
      <div className={styles.argumentsWrapper}>
        <div className={styles.inputWrapper}>
          ...
          {action.inputTypes?.map((inputType: SpotifyActionArgument, index: number) => 
            <DropDownList extraProps={{ label: `${inputType} Input Variable`, onChangeCallback: handleInputDropdownSelect, options: inputVariablesAsDropdownList(validInputVariablesForType(inputType)), selectedOption: getSelectedInputOption(inputType)}} />
          )}
          ...
          {getUndefinedInputTypes().map((inputType: SpotifyActionArgument) => 
            <div className={styles.undefinedInput}>
              {inputType}
            </div>
          )}
        </div>
