...
import MacropadKey from "../MacropadKey/MacropadKey";
import { DeviceKey } from "../../../models/Device";
import {
  ShortcutContextProps,
  ShortcutContext,
} from "../../screens/ShortcutScreen/context";
...
export default function Macropad({ keys, disabled }: MacropadProps) {
  const { selectedDeviceKey, setSelectedDeviceKey } =
    ...
    <div className={styles.deviceWrapper}>
      {keys.map((deviceKey: DeviceKey, index: number) => (
        <MacropadKey
          ...
          onClick={() => handleKeyClick(deviceKey)}
        />
      ))}
    </div>
  );
