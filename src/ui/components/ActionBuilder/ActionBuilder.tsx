...
export default function ActionBuilder() {
  const { actions, setActions } = useContext<CreateShortcutContextProps>(
    CreateShortcutContext
  );
    ...
    <section>
      <div className={styles.container}>
        <div className={styles.actionsWrapper}>
          {actions.map(
            (spotifyAction: SpotifyShortcutAction, index: number) => (
              <ActionBlock
                key={index}
                actionIndex={index}
                actionList={getValidActionList(index)}
                action={spotifyAction}
                setAction={(updatedAction: SpotifyShortcutAction) => {
          ...
          {/* <AddActionBlock /> */}
          {actions.at(actions.length - 1) != UndefinedSpotifyAction && (
            <AddActionBlock />
          )}
        </div>
        <ShortcutVariables />
      ...
      <div className={styles.variablesHeadingWrapper}>
        ...
        <div className={styles.variableSubheading}>
          <Text size={TextSize.SMALL}>
            The outputs from the actions will appear here as variables, to use
            as inputs for other actions.
          </Text>
        </div>
      </div>
