...
export function DeviceBlock({ device }: DeviceBlockProps) {
  const { setSelectedDevice, selectedDeviceKey } =
  ...
  const enrichShortcut = (
    originalShortcut: SpotifyShortcut
  ): SpotifyShortcut => {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.heading}>
        <Heading color={"#404040"} level={HeadingLevel.HEADING2}>
          {device.name}
        </Heading>
      ...
      {selectedDeviceKey?.shortcut && (
        <div className={styles.shortcutWrapper}>
          <div className={styles.shortcutBlockWrapper}>
            <ShortcutBlock
              shortcut={enrichShortcut(selectedDeviceKey.shortcut)}
            />
          </div>
          <div className={styles.inputTypeWrapper}>
            {selectedDeviceKey.shortcut.inputTypes?.map(
              (inputType: SpotifyActionArgument, index: number) => (
                <div className={styles.inputTypeWrapper} key={index}>
                  <Heading level={HeadingLevel.HEADING2} color="white">
                    {inputType}
                  </Heading>
                  <div className={styles.shortcutInputVariablesWrapper}>
                    <div className={styles.selected}>
                      <p>
                        {
                          selectedDeviceKey.shortcut?.inputs?.getVariable(
                            inputType
                          )?.label
                        }
                      </p>
