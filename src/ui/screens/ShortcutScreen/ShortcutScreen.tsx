...
import {
  createShortcutMappingRequest,
  createShortcutMappingRequestWithInputs,
} from "../../../utils/ApiUtils";
...
export default function ShortcutScreen() {
  ...
  } = useContext<ShortcutContextProps>(ShortcutContext);
  ...
  const handleNextClick = () => {
      ...
      if (
        ...
        if (inputTypes != undefined && inputTypes.length > 0) {
          if (selectedInputs != undefined) {
            return createShortcutMappingRequestWithInputs(
              String(selectedDeviceKey.id),
              selectedShortcut.id,
          ...
          return createShortcutMappingRequest(
            String(selectedDeviceKey.id),
            selectedShortcut.id,
            selectedShortcut.type
          );
    ...
    if (shortcutStep == ShortcutStep.CONFIRM) {
      ...
      if (
        shortcutMappingRequest != undefined &&
        user != undefined &&
        selectedDevice != undefined
      ) {
    ...
    if (
      selectedShortcut != undefined &&
      selectedShortcut.inputTypes == undefined &&
      nextShortcutStep == ShortcutStep.SELECT_SHORTCUT_INPUTS
    ) {
      nextShortcutStep += 1;
      console.log("nextShortcutStep", nextShortcutStep);
    }
  ...
  return (
    <div className={styles.container}>
      <Header invertColours={true}>
        ...
        <Button
          extraProps={{ label: "LOGOUT" }}
          htmlProps={{ onClick: handleLogout }}
        />
      </Header>
      <Layout centered={true}>
        <div className={styles.titleSectionWrapper}>
          ...
          <Text color="white" size={TextSize.LARGE}>
            Map Spotify shortcuts to the keys on your macropad.
          </Text>
        </div>
        ...
        <div className={styles.buttonsWrapper}>
          {shortcutStep > ShortcutStep.SELECT_KEY && (
            <Button
              extraProps={{ label: "Back", neutral: true }}
              htmlProps={{ onClick: handleBackClick }}
            />
          )}
          <Button
            extraProps={{
              label: shortcutStep < ShortcutStep.CONFIRM ? "Next" : "Confirm",
            }}
            htmlProps={{ onClick: handleNextClick }}
          />
        </div>
      </Layout>
    </div>
